...
namespace Alice {
	...
	void FrameBufferObject::AttachColorBuffer(const char* bufferName, GLenum attachment, int width, int height, GLenum gpu_format, GLenum cpu_format, GLenum data_type) {
		...
		OGL_CALL(glGenTextures(1, &colorBuffer));
		OGL_CALL(glBindTexture(GL_TEXTURE_2D, colorBuffer));
		...
		OGL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, gpu_format, width, height, 0, cpu_format, data_type, NULL));
		OGL_CALL(glBindTexture(GL_TEXTURE_2D, 0));
		OGL_CALL(glFramebufferTexture2D(GL_FRAMEBUFFER, attachment, GL_TEXTURE_2D, colorBuffer, 0));
	...
	void FrameBufferObject::AttachDepthBuffer(const char* bufferName, int width, int height) {
		GLuint depthMap;
		OGL_CALL(glBindFramebuffer(GL_FRAMEBUFFER, mFrameBufferObject));
		OGL_CALL(glGenTextures(1, &depthMap));
		OGL_CALL(glBindTexture(GL_TEXTURE_2D, depthMap));
		OGL_CALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE));
		OGL_CALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE));
		OGL_CALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR));
		OGL_CALL(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR));
		OGL_CALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, width, height, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL));
		OGL_CALL(glBindTexture(GL_TEXTURE_2D, 0));
		OGL_CALL(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, depthMap, 0));
	...
	void FrameBufferObject::Finish() {
		...
		if (nCount > 0) {
			...
			OGL_CALL(glBindFramebuffer(GL_FRAMEBUFFER, mFrameBufferObject));
			OGL_CALL(glDrawBuffers(nCount, buffers));
			OGL_CALL(glBindFramebuffer(GL_FRAMEBUFFER, 0));
		}
	}
